...
// Je lui aujoute une fonction lors de l'évènement "onreadystatechange"
...
function handleRequest() {
    ...
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        let success, data;
        /* 
            Le "try{} catch(e){}"" permet de mettre un morceau de code dans les accolades du try, qui sera executé, 
            ...
            Optionnellement, on peut ajouter à la fin un "finally" qui executera son code une fois le "try catch" terminé.
        */
        try {
...
/* 
    ...
    On pourra prendre un second paramètre, en cas d'option supplémentaire.

    fetch est suivi d'un ".then()" qui contiendra la fonction callback à lancer une fois la requête terminée.
*/
...
function handleFetch(response) {
    ...
    if (response.ok) {
        /* 
            ...
            Et d'un ".catch()" contenant la fonction callback à lance s'il y a une erreur dans le traitement du JSON
        */
       response.json()
        .then(data=>{
