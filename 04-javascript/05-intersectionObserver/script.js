...
/* 
    ...
    Histoire d'alléger la page si c'est un évènement gourmand.
...
/* 
    ...
    Elle va récupérer en argument, un tableau contenant une entré pour chaque élément observé ou quittant le viewport.
*/
function setIndicator(entries) {
    /* 
        Au chargement de la page, la fonction est lancé une première fois.
        Dans ce premier lancement, le tableau contient tous les éléments observés, indiquans s'ils sont déjà visible ou non.
    ...
    */
    ...
    /* 
        Si on regarde ce que contient cet objet "entry", on trouvera plusieurs informations utiles.
            -target : l'élément qui a été observé (Dans la console)
        ...
        Ici, on va lancer un évènement lorsque le main est visible et retirer l'évènement lorsque le main n'est plus visible.
    */
    console.log(entry);
...
function indicatorAnimation() {
    console.log("SCROOOOOOOOLL");
    /* 
        "scrollY" représente le nombre de pixel qui ont été scrollés.
    ...
    */
    if (window.scrollY > main.offsetTop) {
        //console.log("true");
        /* 
            "scrollHeight" représente la hauteur de l'élément inculant le padding vertical.
...
/* 
    ...
    Si on a plusieurs observation au fonctionnement totalement différent, il vaut mieux faire un nouveau observer pour chacun.
